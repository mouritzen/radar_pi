...
FurunoDRSControl::FurunoDRSControl(NetworkAddress sendAddress) {
  ...
  m_pi = 0;
  m_ri = 0;
  m_name = wxT("FurunoDRS");
}

FurunoDRSControl::~FurunoDRSControl() {
...
bool FurunoDRSControl::Init(radar_pi *pi, RadarInfo *ri, NetworkAddress &ifadr, NetworkAddress &radaradr) {
  int r;
  int one = 1;
...
void FurunoDRSControl::logBinaryData(const wxString &what, const void *data, int size) {
  wxString explain;
...
bool FurunoDRSControl::TransmitCmd(const void *msg, int size) {
  if (m_radar_socket == INVALID_SOCKET) {
...
void FurunoDRSControl::RadarTxOff() {
...
void FurunoDRSControl::RadarTxOn() {
...
bool FurunoDRSControl::RadarStayAlive() {
  // Garmin radars don't need a ping
  return true;
}

bool FurunoDRSControl::SetRange(int meters) {
...
bool FurunoDRSControl::SetControlValue(ControlType controlType, RadarControlItem &item, RadarControlButton *button) {
  bool r = false;
