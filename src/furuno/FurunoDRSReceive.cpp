...
//
//
#define SCALE_RAW_TO_DEGREES(raw) ((raw) * (double)DEGREES_PER_ROTATION / FURUNO_DRS_SPOKES)
#define SCALE_DEGREES_TO_RAW(angle) ((int)((angle) * (double)FURUNO_DRS_SPOKES / DEGREES_PER_ROTATION))
...
struct radar_line {
  ...
  uint16_t mystery2;
  uint16_t mystery3;
  uint8_t line_data[FURUNO_DRS_MAX_SPOKE_LEN * 2];
};
...
//
void FurunoDRSReceive::ProcessFrame(radar_line *packet) {
  ...
  uint8_t line[FURUNO_DRS_MAX_SPOKE_LEN];
  int i;
  uint8_t *p, *s;

  int angle_raw = packet->scan_line;
  int spoke = angle_raw;
  ...
  if (m_next_spoke >= 0 && spoke != m_next_spoke) {
    if (spoke > m_next_spoke) {
      ...
      m_ri->m_statistics.missing_spokes += FURUNO_DRS_SPOKES + spoke - m_next_spoke;
    }
  }

  m_ri->m_state.Update(RADAR_TRANSMIT);
  m_ri->m_range.Update(5556 + 1);
  m_ri->m_gain.Update(100, RCS_AUTO_1);
  m_ri->m_rain.Update(101, RCS_AUTO_1);
  m_ri->m_rain.Update(102);
  m_ri->m_bearing_alignment.Update(0);
  m_ri->m_ftc.Update(1);
  m_ri->m_interference_rejection.Update(0);
  m_ri->m_scan_speed.Update(24);
  ...
  m_next_spoke = (spoke + 1) % FURUNO_DRS_SPOKES;
  ...
  m_ri->ProcessRadarSpoke(a, b, line, p - line, 5556, time_rec);

}

// Check that this interface is valid for
// Garmin HD radar, e.g. is on the same network.
// We know that the radar is on 172.31.3.0 and that
// the netmask is 12 bits, eg 255.240.0.0.

bool FurunoDRSReceive::IsValidFurunoAddress(struct ifaddrs *nif) {
  if (VALID_IPV4_ADDRESS(nif)) {
    ...
    static uint32_t radar = IPV4_ADDR(172, 31, 3, 0);
    static uint32_t radarmask = IPV4_ADDR(172, 31, 0, 0);

    if ((addr & mask) == radarmask && (radar & mask) == radarmask) {
      LOG_RECEIVE(wxT("%s found furuno addr=%X mask=%X req=%X"), m_ri->m_name.c_str(), addr, mask, radarmask);
      return true;
    }
    LOG_RECEIVE(wxT("%s not furuno addr=%X mask=%X req=%X"), m_ri->m_name.c_str(), addr, mask, radarmask);
  }
  return false;
}

SOCKET FurunoDRSReceive::PickNextEthernetCard() {
  ...
  while (m_interface && !IsValidFurunoAddress(m_interface)) {
    m_interface = m_interface->ifa_next;
  }
  if (!m_interface) {
...
SOCKET FurunoDRSReceive::GetNewReportSocket() {
  SOCKET socket;
  wxString error;
 ...
 */
void *FurunoDRSReceive::Entry(void) {
  int r = 0;
  ...
  LOG_VERBOSE(wxT("FurunoDRSReceive thread %s starting"), m_ri->m_name.c_str());
...
bool FurunoDRSReceive::UpdateScannerStatus(int status) {
  bool ret = true;
...
bool FurunoDRSReceive::ProcessReport(const uint8_t *report, size_t len) {
...
// this message ready for it to be read on 'm_receive_socket'. See the constructor in FurunoDRSReceive.h
// for the setup of these two sockets.

void FurunoDRSReceive::Shutdown() {
  if (m_send_socket != INVALID_SOCKET) {
...
wxString FurunoDRSReceive::GetInfoStatus() {
  wxCriticalSectionLocker lock(m_lock);
